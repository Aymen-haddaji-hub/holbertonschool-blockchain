 ...
 * @node: current node
 * @idx: index of node
 * @arg: Collect_sender_unspent struct
 * Return: 0 if continue else 1
 */
int collect_sender_unspent(llist_node_t node, unsigned int idx, void *arg)
{
	collect_sender_unspent_t *visitor = arg;
	unspent_tx_out_t *utx = node;

	(void) idx;

	if (!memcmp(utx->out.pub, visitor->sender_pubkey, EC_PUB_LEN))
	{
...
/**
 * Txo_to_Txi - maps output to input txs
 * @node: current node
 * @idx: index of node
 * @arg: inputs list
 * Return: 0 if continue else 1
 */
int Txo_to_Txi(llist_node_t node, unsigned int idx, void *arg)
{
	llist_t *inputs = arg;
	unspent_tx_out_t *utx = node;
...
/**
 * fill_tx_data - populates tx struct with data
 * @sender: sender's key
 ...
 * Return: tx struct
 */
transaction_t *fill_tx_data(EC_KEY const *sender,
		collect_sender_unspent_t *visitor,
		...
		transaction_t *tx)
{
	...
	llist_for_each(visitor->sender_unspent, Txo_to_Txi, tx->inputs);
		...
		llist_t *all_unspent)
{
	...
	collect_sender_unspent_t visitor = {0};
	...
	visitor.sender_pubkey = sender_pub;
	...
	return (fill_tx_data(sender, &visitor, all_unspent, sender_pub,
				receiver_pub, tx));
}
